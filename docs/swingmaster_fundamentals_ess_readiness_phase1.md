...
Recommended next task: Phase 4, reported fundamentals point-in-time/vintage design.

Phase 3 now provides read-only visibility into current table readiness without provider calls, schema changes, DB writes, or ESS decision integration. The next design pressure is reported-fundamental availability and vintage metadata: `available_at_utc`, first-class `filed_at_utc`, `statement_vintage_id`, source hashing, and restatement/revision handling.
