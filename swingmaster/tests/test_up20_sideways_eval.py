...
from swingmaster.cli import run_up20_sideways_eval
...
from swingmaster.research.up20_sideways_evaluation import (
    EVAL_FILE_NAME,
    _compute_threshold_metrics,
    _compute_top_bucket_metrics,
    _recommend_candidate,
    evaluate_up20_sideways_models,
)
from swingmaster.research.up20_sideways_training import load_up20_sideways_dataset, train_and_compare_up20_sideways
...
def test_up20_sideways_eval_uses_same_sideways_only_dataset_logic(tmp_path: Path) -> None:
    ...
    conn = sqlite3.connect(str(rc_db))
    eval_summary = evaluate_up20_sideways_models(
        conn,
        model_dir=model_dir,
        regime_version=regime_version,
        computed_at="2026-03-07T00:00:00+00:00",
...
def test_up20_sideways_eval_uses_same_feature_order_as_training(tmp_path: Path) -> None:
    rc_db = tmp_path / "rc.db"
    conn = sqlite3.connect(str(rc_db))
    apply_migrations(conn)
    regime_version = "REGIME_TEST_V1"
    ...
    feature_path = model_dir / "UP20_SIDEWAYS_FEATURE_LIST_V1.json"
    ...
    conn = sqlite3.connect(str(rc_db))
    with pytest.raises(RuntimeError, match="UP20_SIDEWAYS_FEATURE_ORDER_MISMATCH"):
        evaluate_up20_sideways_models(
            conn,
            model_dir=model_dir,
            regime_version=regime_version,
...
def test_up20_sideways_eval_computes_threshold_metrics() -> None:
...
def test_up20_sideways_eval_computes_top_bucket_metrics() -> None:
...
def test_up20_sideways_eval_cli_runs_and_writes_eval_json(monkeypatch, tmp_path: Path) -> None:
    rc_db = tmp_path / "rc.db"
    conn = sqlite3.connect(str(rc_db))
    apply_migrations(conn)
    regime_version = "REGIME_TEST_V1"
    ...
    run_up20_sideways_eval.main()

    assert (out_dir / EVAL_FILE_NAME).exists()


def test_up20_sideways_eval_recommendation_is_deterministic() -> None:
    kwargs = {
        "previous_selected": "UP20_SIDEWAYS_CATBOOST_V1",
