...
from swingmaster.research.up20_sideways_training import (
    COMPARISON_ID,
    MODEL_ID_CATBOOST,
    MODEL_ID_HGB,
    Up20SidewaysDataset,
    load_up20_sideways_dataset,
)

EVAL_ID = "UP20_SIDEWAYS_MODEL_EVAL_V1"
EVAL_FILE_NAME = f"{EVAL_ID}.json"
FEATURE_LIST_FILE_NAME = "UP20_SIDEWAYS_FEATURE_LIST_V1.json"
...
def _load_feature_list(model_dir: Path) -> list[str]:
    ...
    if not isinstance(cols, list) or not all(isinstance(x, str) for x in cols):
        raise RuntimeError("UP20_SIDEWAYS_FEATURE_LIST_INVALID")
    return cols
...
def _recommend_candidate(
    *,
    ...
    practical_order = [
        ...
        ("gt_50_rate_top_10pct_test", gt50_top10_catboost, gt50_top10_hgb),
    ]
...
def _load_models(model_dir: Path) -> tuple[CatBoostClassifier, Any]:
    ...
    if not cat_path.exists():
        raise RuntimeError("UP20_SIDEWAYS_CATBOOST_ARTIFACT_MISSING")
    if not hgb_path.exists():
        raise RuntimeError("UP20_SIDEWAYS_HGB_ARTIFACT_MISSING")
...
def _build_eval_frame(
    ...
    *,
    ...
    ds = load_up20_sideways_dataset(conn, regime_version=regime_version)
    if ds.frame.empty:
        raise RuntimeError("UP20_SIDEWAYS_DATASET_EMPTY")
    if tuple(feature_order_from_artifact) != ds.feature_columns:
        raise RuntimeError("UP20_SIDEWAYS_FEATURE_ORDER_MISMATCH")
    return ds


def evaluate_up20_sideways_models(
    conn: sqlite3.Connection,
    *,
    model_dir: Path,
    ...
    if valid_df.empty or test_df.empty:
        raise RuntimeError("UP20_SIDEWAYS_SPLIT_EMPTY")
    ...
    return Up20SidewaysEvalSummary(
        n_valid=int(valid_df.shape[0]),
        n_test=int(test_df.shape[0]),
        pos_rate_valid=float(np.mean(y_valid)),
        pos_rate_test=float(np.mean(y_test)),
