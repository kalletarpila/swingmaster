...
from swingmaster.fundamentals_v2.legacy_yahoo_ebit import run_legacy_yahoo_ebit_import


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Offline legacy Yahoo raw direct EBIT importer for rc_fundamentals_v2")
    ...
    parser.add_argument("--apply", action="store_true", help="Apply NULL-only EBIT fills")
...
def main() -> None:
    ...
    if args.dry_run and args.apply:
        raise SystemExit("LEGACY_YAHOO_EBIT_DRY_RUN_AND_APPLY_MUTUALLY_EXCLUSIVE")
    if not args.dry_run and not args.apply:
        raise SystemExit("LEGACY_YAHOO_EBIT_REQUIRES_DRY_RUN_OR_APPLY")
