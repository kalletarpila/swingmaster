...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Rebuild TTM effective-date metadata from quarter earnings matches")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.apply and not args.backup and not _existing_effective_metadata_populated(db_path):
    ...
    before_effective_hash: str
    after_effective_hash: str
    before_financial_hash: str
    after_financial_hash: str
    apply_counts = {"inserted": 0, "financial_value_updates": 0, "effective_date_updates": 0, "unchanged": 0}

    if args.apply and args.backup:
        backup_path = output_root / "backups" / f"fundamentals_usa.pre_ttm_effective_date.{utc_timestamp()}.db"
    ...
    with sqlite3.connect(str(db_path)) as conn:
        ...
        before_effective_hash = effective_fields_hash(conn)
        before_financial_hash = financial_fields_hash(conn)
        rows = compute_ttm_effective_dates(conn, tickers)
        summary = summarize(rows)
        if args.apply:
            with conn:
                apply_counts = apply_effective_date_rows(conn, rows)
        after_effective_hash = effective_fields_hash(conn)
        after_financial_hash = financial_fields_hash(conn)
    ...
    payload = {
        ...
        "backup": backup_info,
        "effective_hash_before": before_effective_hash,
        "effective_hash_after": after_effective_hash,
        "financial_hash_before": before_financial_hash,
        "financial_hash_after": after_financial_hash,
        "financial_values_unchanged": before_financial_hash == after_financial_hash,
    ...
    if quick_check != "ok" or pre_counts != post_counts or before_financial_hash != after_financial_hash:
        return 2
    return 0
...
def _output_root(value: str | None, *, dry_run: bool) -> Path:
    if value:
        ...
        root = repository_root() / "temp" / "fundamental_ttm_effective_date" / utc_timestamp() / ("dry_run" if dry_run else "apply")
...
def _existing_effective_metadata_populated(db_path: Path) -> bool:
    with sqlite3.connect(str(db_path)) as conn:
        columns = {str(row[1]) for row in conn.execute("PRAGMA table_info(rc_fundamental_ttm)")}
        if not {"effective_date_status", "effective_date_policy"}.issubset(columns):
            return False
        row = conn.execute(
            """
            SELECT COUNT(*), SUM(CASE WHEN effective_date_status IS NOT NULL THEN 1 ELSE 0 END)
            FROM rc_fundamental_ttm
            """
        ).fetchone()
...
def _write_artifacts(
    ...
    output_csv = validate_temp_path(Path(args.output_csv)) if args.output_csv else output_root / "ttm_effective_dates.csv"
    write_json_atomic(checkpoint, payload)
    write_json_atomic(summary_json, {key: payload[key] for key in ("mode", "database_path", "pre_counts", "post_counts", "source_table_counts_unchanged", "quick_check", "summary", "apply_counts", "backup", "verification", "financial_values_unchanged", "effective_hash_before", "effective_hash_after")})
...
def _print_summary(payload: dict[str, Any], artifact_paths: dict[str, str]) -> None:
    ...
    print(f"financial_values_unchanged: {payload['financial_values_unchanged']}")
