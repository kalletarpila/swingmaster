...
from swingmaster.cli import run_fail10_bull_score
...
from swingmaster.research import fail10_bull_training
from swingmaster.research.fail10_bull_scoring import (
    SCORE_TABLE,
...
def _write_model_artifacts(
    ...
    *,
    ...
    selected_candidate: str = "FAIL10_BULL_HGB_V1",
) -> None:
    model_dir.mkdir(parents=True, exist_ok=True)
    feature_columns = feature_order if feature_order is not None else list(FEATURE_COLUMNS)
    (model_dir / "FAIL10_BULL_FEATURE_LIST_V1.json").write_text(
        json.dumps({"feature_columns": feature_columns, "feature_count": len(feature_columns)}, indent=2, sort_keys=True)
        + "\n",
        encoding="utf-8",
    )
    (model_dir / "FAIL10_BULL_MODEL_EVAL_V1.json").write_text(
        json.dumps(
            {
                "previous_selected_production_candidate": "FAIL10_BULL_HGB_V1",
    ...
    joblib.dump(DummyProbModel(), model_dir / "FAIL10_BULL_HGB_V1.joblib")


def test_fail10_bull_scoring_filters_to_bull_regime_only(tmp_path: Path) -> None:
    ...
    conn = sqlite3.connect(str(rc_db))
    summary = compute_and_store_fail10_bull_scores(
        conn,
        model_dir=model_dir,
        regime_version=regime_version,
        mode="upsert",
...
def test_fail10_bull_scoring_uses_saved_feature_order(tmp_path: Path, monkeypatch) -> None:
    rc_db = tmp_path / "rc.db"
    conn = sqlite3.connect(str(rc_db))
    apply_migrations(conn)
    regime_version = "REGIME_TEST_V1"
    ...
    from swingmaster.research import fail10_bull_scoring

    monkeypatch.setattr(
        fail10_bull_scoring,
        "_load_model",
        lambda _model_dir, _model_id: (SpyModel(), "HGB", str((model_dir / "FAIL10_BULL_HGB_V1.joblib").resolve())),
    )
...
def test_fail10_bull_scoring_writes_probability_to_score_table(tmp_path: Path) -> None:
    rc_db = tmp_path / "rc.db"
    conn = sqlite3.connect(str(rc_db))
    apply_migrations(conn)
    regime_version = "REGIME_TEST_V1"
    ...
    model_dir = tmp_path / "models"
    _write_model_artifacts(model_dir, selected_candidate="FAIL10_BULL_HGB_V1")
    ...
    assert len(rows) == 2
    assert all(str(r[1]) == "FAIL10_BULL_HGB_V1" for r in rows)
    assert all(str(r[2]) == "BULL" for r in rows)
    assert all(str(r[3]) == "FAIL10" for r in rows)
    assert all(0.0 <= float(r[4]) <= 1.0 for r in rows)


def test_fail10_bull_scoring_cli_is_idempotent(monkeypatch, tmp_path: Path) -> None:
    rc_db = tmp_path / "rc.db"
    conn = sqlite3.connect(str(rc_db))
    apply_migrations(conn)
    regime_version = "REGIME_TEST_V1"
    ...
    monkeypatch.setattr(run_fail10_bull_score, "parse_args", lambda: args)

    run_fail10_bull_score.main()
    run_fail10_bull_score.main()
...
def test_fail10_bull_scoring_fails_on_feature_order_mismatch(tmp_path: Path) -> None:
    rc_db = tmp_path / "rc.db"
    conn = sqlite3.connect(str(rc_db))
    apply_migrations(conn)
    regime_version = "REGIME_TEST_V1"
...
def test_fail10_bull_scoring_does_not_retrain_model(tmp_path: Path, monkeypatch) -> None:
    rc_db = tmp_path / "rc.db"
    conn = sqlite3.connect(str(rc_db))
    apply_migrations(conn)
    regime_version = "REGIME_TEST_V1"
