...
| Phase 4 | Reported fundamentals point-in-time/vintage hardening. Phase 4A design is documented in [Reported Fundamentals PIT/Vintage Design Phase 4A](swingmaster_reported_fundamentals_pit_vintage_design_phase4a.md); no migration has been implemented yet. | Migrations and provider persistence paths after separate design. | Add `available_at`, vintage/restatement metadata only after migration design. | Migration tests, SEC/Yahoo fixture tests, backfill safety tests. | No scoring/valuation changes except reading stable inputs. | Reported rows can represent multiple vintages without lookahead. |
...
Recommended next task: Phase 4B, reported fundamentals PIT/vintage schema migration design with temp-DB tests only.

Phase 4A now defines the target reported-fundamentals availability and vintage model without adding migrations. The next safe step is an additive migration design and temp-DB-only tests for `available_at_utc`, first-class `filed_at_utc`, `statement_vintage_id`, source hashing, and restatement/revision handling.
