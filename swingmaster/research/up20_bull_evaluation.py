...
from swingmaster.research.up20_bull_training import (
    COMPARISON_ID,
    MODEL_ID_CATBOOST,
    MODEL_ID_HGB,
    Up20BullDataset,
    load_up20_bull_dataset,
)

EVAL_ID = "UP20_BULL_MODEL_EVAL_V1"
EVAL_FILE_NAME = f"{EVAL_ID}.json"
FEATURE_LIST_FILE_NAME = "UP20_BULL_FEATURE_LIST_V1.json"
    ...
    *,
    ...
    for name, frac in buckets:
        ...
        out[name] = {
            ...
            "up20_rate": _to_nullable(np.mean(y_bucket)),
            ...
            "gt_100_rate_bucket": _to_nullable(np.mean(g_bucket > 100.0)),
        }
    return out
...
def _recommend_candidate(
    *,
    ...
    auc_test_catboost: float | None,
    auc_test_hgb: float | None,
    top10_up20_catboost: float | None,
    top10_up20_hgb: float | None,
    avg_growth_top10_catboost: float | None,
    avg_growth_top10_hgb: float | None,
    precision_060_catboost: float | None,
    precision_060_hgb: float | None,
    ...
    practical_order = [
        ("top_10pct_up20_rate_test", top10_up20_catboost, top10_up20_hgb),
        ("avg_growth_top_10pct_test", avg_growth_top10_catboost, avg_growth_top10_hgb),
        ("precision_at_0_60_test", precision_060_catboost, precision_060_hgb),
    ]
    practical_winner: str | None = None
    practical_metric = "none"
    for metric_name, c_val, h_val in practical_order:
        cmp_val = _cmp(c_val, h_val)
        if cmp_val > 0:
            practical_winner = MODEL_ID_CATBOOST
            practical_metric = metric_name
            break
        if cmp_val < 0:
            practical_winner = MODEL_ID_HGB
            practical_metric = metric_name
            break

    if practical_winner is not None:
        return practical_winner, {
            "baseline_from_auc_test": baseline,
            "practical_winner_metric": practical_metric,
    ...
    *,
    ...
    return {
        ...
        "big_winner_valid": {
            ...
            "gt_100_rate_top_10pct": top_valid["top_10pct"]["gt_100_rate_bucket"],
        },
        "big_winner_test": {
            ...
            "gt_100_rate_top_10pct": top_test["top_10pct"]["gt_100_rate_bucket"],
        },
    }
...
def _load_models(model_dir: Path) -> tuple[CatBoostClassifier, Any]:
    ...
    if not cat_path.exists():
        raise RuntimeError("UP20_BULL_CATBOOST_ARTIFACT_MISSING")
...
def evaluate_up20_bull_models(
    ...
    *,
    ...
    ds = _build_eval_frame(conn, regime_version=regime_version, feature_order_from_artifact=feature_order)
    frame = ds.frame

    valid_df = frame[frame["split_bucket"] == "valid"].copy()
    test_df = frame[frame["split_bucket"] == "test"].copy()
    if valid_df.empty or test_df.empty:
        raise RuntimeError("UP20_BULL_SPLIT_EMPTY")

    x_valid = valid_df[feature_order]
    x_test = test_df[feature_order]
    y_valid = valid_df["label_up20"].astype(int).to_numpy()
    y_test = test_df["label_up20"].astype(int).to_numpy()
    ...
    recommended, decision_meta = _recommend_candidate(
        ...
        top10_up20_catboost=cat_eval["top_bucket_metrics_test"]["top_10pct"]["up20_rate"],
        top10_up20_hgb=hgb_eval["top_bucket_metrics_test"]["top_10pct"]["up20_rate"],
        ...
        precision_060_catboost=cat_eval["threshold_metrics_test"]["0.60"]["precision"],
        precision_060_hgb=hgb_eval["threshold_metrics_test"]["0.60"]["precision"],
    )
    ...
    payload = {
        ...
        "comparison_test_practical": {
            ...
            "top_10pct_up20_rate_test": {
                MODEL_ID_CATBOOST: cat_eval["top_bucket_metrics_test"]["top_10pct"]["up20_rate"],
                MODEL_ID_HGB: hgb_eval["top_bucket_metrics_test"]["top_10pct"]["up20_rate"],
            },
    ...
    return Up20BullEvalSummary(
        ...
        auc_test_hgb=hgb_eval["auc_test"],
        precision_at_0_60_valid_catboost=cat_eval["threshold_metrics_valid"]["0.60"]["precision"],
        ...
        top_10pct_up20_rate_test_catboost=cat_eval["top_bucket_metrics_test"]["top_10pct"]["up20_rate"],
        ...
        precision_at_0_60_valid_hgb=hgb_eval["threshold_metrics_valid"]["0.60"]["precision"],
        ...
        top_10pct_up20_rate_test_hgb=hgb_eval["top_bucket_metrics_test"]["top_10pct"]["up20_rate"],
