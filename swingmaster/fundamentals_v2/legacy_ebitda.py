...
SOURCE_DATASET = "legacy_yahoo_raw_quarterly_income_stmt"
SOURCE_TABLE = "rc_fundamental_yahoo_raw"
BUILDER_VERSION = "legacy_yahoo_ebitda_fallback_v1"
TRANSFORMATION = "none"
SEMANTIC_DEFINITION = "direct EBITDA from Yahoo quarterly income statement"
...
def parse_yahoo_ebitda_observations(payload_json: str, *, ticker: str, raw_id: int, payload_hash: str, run_id: str, loaded_at_utc: str) -> list[YahooEbitdaObservation]:
    ...
    if len(rows) > 1:
        raise ValueError("YAHOO_EBITDA_DUPLICATE_INDEX")
    if not rows:
        return []
    values = data[rows[0]] if rows[0] < len(data) else []
    observations: list[YahooEbitdaObservation] = []
    for idx, period_end_date in enumerate(columns):
        if idx >= len(values) or values[idx] is None:
            continue
        observations.append(YahooEbitdaObservation(ticker.upper(), period_end_date, float(values[idx]), raw_id, payload_hash, run_id, loaded_at_utc))
    return observations
...
def _load_v2_rows(conn: sqlite3.Connection, *, market: str) -> list[V2Quarter]:
    return [
        V2Quarter(
            ...
            depreciation_amortization=None if row["depreciation_amortization"] is None else float(row["depreciation_amortization"]),
        ...
        for row in conn.execute(
            ...
            SELECT c.ticker, c.company_id, q.quarter_id, q.fiscal_year, q.fiscal_period, q.report_date,
                   f.revenue, f.ebitda, f.free_cashflow, f.operating_income, f.depreciation_amortization,
...
def load_yahoo_ebitda_observations(conn: sqlite3.Connection, *, market: str) -> tuple[dict[str, list[YahooEbitdaObservation]], list[dict[str, Any]]]:
    seen: dict[tuple[str, str], dict[float, YahooEbitdaObservation]] = defaultdict(dict)
    conflicts: list[dict[str, Any]] = []
    for row in conn.execute(
        """
        SELECT id, symbol, quarterly_income_stmt_json, payload_hash, run_id, loaded_at_utc
        ...
        try:
            observations = parse_yahoo_ebitda_observations(
                str(row["quarterly_income_stmt_json"]),
    ...
    by_ticker: dict[str, list[YahooEbitdaObservation]] = defaultdict(list)
    for (ticker, period), values in seen.items():
        if len(values) != 1:
            conflicts.append({"ticker": ticker, "period_end_date": period, "classification": "CONFLICTING_YAHOO_EBITDA_VALUES", "value_count": len(values)})
            continue
...
def classify_companies(v2_rows: list[V2Quarter], yahoo_by_ticker: dict[str, list[YahooEbitdaObservation]]) -> tuple[list[dict[str, Any]], dict[str, str]]:
    ...
    for ticker in sorted(set(yahoo_by_ticker) | {row.ticker for row in v2_rows}):
        ...
        for obs in yahoo_by_ticker.get(ticker, []):
            ...
            if v2 is None or v2.ebitda is None:
                continue
            ratio = obs.value / v2.ebitda if v2.ebitda else None
            overlaps.append((_relative_difference(obs.value, v2.ebitda), ratio, obs.value, v2.ebitda))
        ...
        if len(overlaps) >= 3 and not scaling and not sign and (_percentile(rels, 0.9) or 1.0) <= 0.02 and max(rels) <= 0.10:
            ...
            tier = "ACCEPTED_RISK"
            consistency = "plausible_direct_ebitda_with_divergence_tail_or_sparse_overlap"
        else:
            tier = "DO_NOT_USE"
        ...
        rows.append(
            {
                ...
                "risk_tier": tier,
                "validation_scope": "company+Yahoo direct EBITDA",
            }
        )
    return rows, tiers


def build_recoverability(v2_rows: list[V2Quarter], yahoo_by_ticker: dict[str, list[YahooEbitdaObservation]], tiers: dict[str, str]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for row in v2_rows:
        ...
        if row.operating_income is not None and row.depreciation_amortization is not None:
            ...
            offset = 0
            tier = "SAFE_SCOPED"
            value: Any = row.operating_income + row.depreciation_amortization
        else:
        ...
        historical.append(
            {
                ...
                "formula": "operating_income + depreciation_amortization" if category == "INTERNAL_V2_RECOVERY" else "Yahoo direct EBITDA",
                "source_fields": "operating_income|depreciation_amortization" if category == "INTERNAL_V2_RECOVERY" else PROVIDER_FIELD if obs else "",
                "candidate_ebitda": value,
                ...
                "category": category,
                "validation_rule": "internal V2 canonical formula" if category == "INTERNAL_V2_RECOVERY" else "company-scoped Yahoo direct EBITDA",
                "sec_context": "",
    ...
    return json.dumps(
        {
            ...
            "formula": row["formula"],
            "sign_transformation": "none; Yahoo direct EBITDA and direct EBITDA value preserved",
            "provider_value": row["candidate_ebitda"],
            ...
            "risk_note": "accepted internal-use risk for deterministic Yahoo direct EBITDA after company-level overlap checks"
...
def apply_ebitda_rows(conn: sqlite3.Connection, rows: list[dict[str, Any]], *, run_id: str, dry_run: bool, now: str | None = None) -> list[dict[str, Any]]:
    now = now or utc_now()
    ...
    results = []
    for row in rows:
        current = conn.execute("SELECT ebitda FROM rc_v2_fundamental_quarterly WHERE quarter_id=?", (row["quarter_id"],)).fetchone()
        if current is None:
            action = "REJECT_MISSING_V2_QUARTER"
        elif current["ebitda"] is not None:
            action = "SAME_VALUE_NOOP" if float(current["ebitda"]) == float(row["candidate_ebitda"]) else "CONFLICT_EXISTING_DIFFERENT"
        else:
            ...
            if not dry_run:
                conn.execute(
                    ...
                    SET ebitda=?, available_canonical_field_count=available_canonical_field_count+1, updated_at_utc=?
                    WHERE quarter_id=? AND ebitda IS NULL
                    """,
                    (row["candidate_ebitda"], now, row["quarter_id"]),
                )
                conn.execute(
                    """
                    ...
                    (
                        ...
                        TRANSFORMATION if row["provider"] == PROVIDER else "operating_income + depreciation_amortization",
                        _source_value(row),
                        run_id,
                        now,
                    ),
...
def run_legacy_ebitda_import(
    *,
    ...
    try:
        ...
        yahoo_by_ticker, yahoo_conflicts = load_yahoo_ebitda_observations(legacy_conn, market=market)
        ...
        results = apply_ebitda_rows(v2_conn, eligible, run_id=run_id, dry_run=dry_run, now=utc_now())
        if apply and not dry_run:
            v2_conn.commit()
        after = _snapshot(v2_conn, eligible)
        replay = apply_ebitda_rows(v2_conn, eligible, run_id=run_id, dry_run=True, now=utc_now())
        ...
        continuity_rows = [
            {"tier_scope": "before", "latest_4q_complete": base["latest_4q_complete_ebitda"], "latest_8q_complete": base["latest_8q_complete_ebitda"]},
...
def _write_artifacts(artifact_dir: Path, v2_rows: list[V2Quarter], legacy_conn: sqlite3.Connection, yahoo_by_ticker: dict[str, list[YahooEbitdaObservation]], quality_rows: list[dict[str, Any]], recoverability: list[dict[str, Any]], latest_recoverability: list[dict[str, Any]], eligible: list[dict[str, Any]], before: list[dict[str, Any]], results: list[dict[str, Any]], after: list[dict[str, Any]], replay: list[dict[str, Any]], base: dict[str, Any], continuity_rows: list[dict[str, Any]], downstream_rows: list[dict[str, Any]], before_integrity: dict[str, Any], after_integrity: dict[str, Any], yahoo_conflicts: list[dict[str, Any]], tier_counts: Counter, recovery_counts: Counter, latest_counts: Counter, backup: dict[str, Any] | None, apply: bool) -> None:
    write_csv(artifact_dir / "ebitda_baseline.csv", [base])
    (artifact_dir / "current_ebitda_semantics.md").write_text(
        "# Current V2 EBITDA Semantics\n\n- Formula: `ebitda = operating_income + depreciation_amortization`.\n- Canonical operating income source: SimFin operating income from the income statement.\n- Canonical D&A source hierarchy: SimFin cash-flow D&A plus the validated PL D&A fallback when available.\n- NULL behavior: EBITDA is NULL unless both canonical operating income and D&A are available at build/fallback time.\n- Provenance: derived SimFin/V2 field provenance records source fields and transformation.\n",
        encoding="utf-8",
    )
    write_csv(artifact_dir / "yahoo_ebitda_candidates_refreshed.csv", _yahoo_inventory(legacy_conn))
    write_csv(artifact_dir / "sec_ebitda_components_refreshed.csv", _sec_inventory(legacy_conn))
    write_csv(artifact_dir / "ebitda_candidate_formulas_pragmatic.csv", _formula_rows())
    write_csv(artifact_dir / "ebitda_overlap_risk_validation.csv", _overlap_rows(v2_rows, yahoo_by_ticker))
    write_csv(artifact_dir / "ebitda_adjustment_semantics_pragmatic.csv", _yahoo_formula_validation(legacy_conn))
    write_csv(artifact_dir / "ebitda_candidate_risk_tiers.csv", quality_rows)
    write_csv(artifact_dir / "ebitda_cross_provider_corroboration_pragmatic.csv", [])
    write_csv(artifact_dir / "ebitda_historical_recoverability_pragmatic.csv", recoverability)
    write_csv(artifact_dir / "ebitda_recoverability_by_year_and_tier.csv", _by_year(recoverability))
    write_csv(artifact_dir / "ebitda_continuity_payoff_pragmatic.csv", continuity_rows)
    write_csv(artifact_dir / "latest_quarter_ebitda_recoverability_pragmatic.csv", latest_recoverability)
    write_csv(artifact_dir / "ebitda_downstream_payoff_pragmatic.csv", downstream_rows)
    write_csv(artifact_dir / "eligible_ebitda_rows.csv", eligible)
    ...
    write_csv(artifact_dir / "replay_audit.csv", [{"ebitda_delta": sum(1 for row in replay if row["action"] == "WOULD_FILL"), "provenance_delta": 0, "provider_calls": 0}])
    write_csv(artifact_dir / "readiness_impact.csv", downstream_rows)
    decision = {
        "decision": "EBITDA_ACCEPTED_RISK_AVAILABLE" if tier_counts["ACCEPTED_RISK"] else "EBITDA_SAFE_SCOPED_AVAILABLE" if tier_counts["SAFE_SCOPED"] else "EBITDA_DO_NOT_USE",
        ...
        "projected_4q_gain_safe_plus_accepted_risk": continuity_rows[2]["latest_4q_complete"] - base["latest_4q_complete_ebitda"],
        "projected_8q_gain_safe_plus_accepted_risk": continuity_rows[2]["latest_8q_complete"] - base["latest_8q_complete_ebitda"],
    ...
    summary = {
        ...
        "final_phase3_classification": "PHASE_3_EBITDA_BACKFILL_COMPLETE_WITH_ACCEPTED_RISK" if decision["production_financial_writes"] else "PHASE_3_EBITDA_NO_USABLE_FALLBACK",
    }
    write_json(artifact_dir / "summary.json", summary)
    (artifact_dir / "recommended_next_step.md").write_text("Proceed to MASTER PLAN PHASE 7A-R-7D - total_debt P1 pragmatic re-validation and backfill.\n", encoding="utf-8")


def _baseline(v2_rows: list[V2Quarter]) -> dict[str, Any]:
    total = len(v2_rows)
    non = sum(row.ebitda is not None for row in v2_rows)
    ...
    for rows in by_company.values():
        present = sum(row.ebitda is not None for row in rows)
        hist["complete" if present == len(rows) else "none" if present == 0 else "partial"] += 1
    return {"ordinary_canonical_quarters": total, "ebitda_non_null": non, "ebitda_null": total - non, "coverage_pct": round(non / total * 100.0, 4) if total else 0.0, "complete_ebitda_history_companies": hist["complete"], "partial_ebitda_history_companies": hist["partial"], "no_ebitda_history_companies": hist["none"], "latest_ebitda_available": sum(row.ebitda is not None for row in latest.values()), "latest_ebitda_missing": sum(row.ebitda is None for row in latest.values()), "latest_4q_complete_ebitda": _continuity_count(v2_rows, fill_rows=[], quarters=4), "latest_8q_complete_ebitda": _continuity_count(v2_rows, fill_rows=[], quarters=8)}
...
def _continuity_count(v2_rows: list[V2Quarter], *, fill_rows: list[dict[str, Any]] | set[int], quarters: int, tiers: set[str] | None = None) -> int:
    ...
    for row in v2_rows:
        by_company[row.ticker].append(row)
    return sum(1 for rows in by_company.values() if len((tail := sorted(rows, key=lambda row: row.report_date, reverse=True)[:quarters])) == quarters and all(row.ebitda is not None or row.quarter_id in filled for row in tail))
...
def _downstream_payoff(v2_rows: list[V2Quarter], eligible: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    def calc(filled: set[int]) -> dict[str, int]:
        def has_ebitda(row: V2Quarter) -> bool:
            return row.ebitda is not None or row.quarter_id in filled
        return {"P0_readiness": sum(row.revenue is not None and has_ebitda(row) and row.free_cashflow is not None for row in latest_rows), "P0_P1_readiness": sum(row.revenue is not None and has_ebitda(row) and row.free_cashflow is not None and row.cash is not None and row.total_debt is not None and row.shares_outstanding is not None for row in latest_rows), "4Q_TTM_readiness": _continuity_count(v2_rows, fill_rows=filled, quarters=4), "EBITDA_leverage_readiness": sum(has_ebitda(row) and row.cash is not None and row.total_debt is not None for row in latest_rows), "score_readiness": sum(row.revenue is not None and has_ebitda(row) and row.free_cashflow is not None for row in latest_rows), "valuation_readiness": sum(row.revenue is not None and has_ebitda(row) and row.free_cashflow is not None and row.cash is not None and row.total_debt is not None and row.shares_outstanding is not None for row in latest_rows)}
    ...
    for row in after:
        ...
        for field in AUDIT_FIELDS:
            if prior.get(field) != row.get(field):
                changed[field] += 1
    return {"ebitda_changes": changed["ebitda"], "unrelated_field_writes": sum(v for k, v in changed.items() if k in AUDIT_FIELDS and k != "ebitda"), "bank_insurance_writes": 0}


def _integrity(conn: sqlite3.Connection) -> dict[str, Any]:
    return {"integrity_check": conn.execute("PRAGMA integrity_check").fetchone()[0], "foreign_key_check_rows": len(conn.execute("PRAGMA foreign_key_check").fetchall()), "duplicate_quarters": conn.execute("SELECT COUNT(*) FROM (SELECT company_id, fiscal_year, fiscal_period, report_date, COUNT(*) n FROM rc_v2_quarter GROUP BY company_id, fiscal_year, fiscal_period, report_date HAVING n>1)").fetchone()[0], "orphan_provenance": conn.execute("SELECT COUNT(*) FROM rc_v2_fundamental_field_source s LEFT JOIN rc_v2_quarter q ON q.quarter_id=s.quarter_id WHERE q.quarter_id IS NULL").fetchone()[0], "fallback_ebitda_without_provenance": conn.execute("SELECT COUNT(*) FROM rc_v2_fundamental_field_source WHERE field_name='ebitda' AND provider='YAHOO' AND source_value IS NULL").fetchone()[0], "accepted_risk_ebitda_without_metadata": conn.execute("SELECT COUNT(*) FROM rc_v2_fundamental_field_source WHERE field_name='ebitda' AND provider='YAHOO' AND source_value NOT LIKE '%\"validation_tier\": \"ACCEPTED_RISK\"%' AND source_value NOT LIKE '%\"validation_tier\": \"SAFE_SCOPED\"%'").fetchone()[0], "sec_derived_ebitda_missing_context_formula": 0}


def _formula_rows() -> list[dict[str, Any]]:
    return [{"formula": "YAHOO_DIRECT_EBITDA", "source_fields": PROVIDER_FIELD, "formula_expression": "direct", "transformation": "none", "duration_context_requirement": "quarterly provider column matched to canonical quarter", "exclusions": "ambiguous identity, company-level DO_NOT_USE, non-null canonical EBITDA"}, {"formula": "YAHOO_OPERATING_INCOME_PLUS_DA", "source_fields": "Operating Income+Reconciled Depreciation", "formula_expression": "operating income + D&A", "transformation": "none", "duration_context_requirement": "read-only validation only", "exclusions": "not implemented; direct EBITDA selected as smallest production path"}, {"formula": "SEC_OPERATING_INCOME_PLUS_DA", "source_fields": "OperatingIncomeLoss + D&A concepts", "formula_expression": "operating income + D&A", "transformation": "requires duration/context validation", "duration_context_requirement": "clean quarter duration only", "exclusions": "not implemented in this phase due context complexity"}]


def _overlap_rows(v2_rows: list[V2Quarter], yahoo_by_ticker: dict[str, list[YahooEbitdaObservation]]) -> list[dict[str, Any]]:
    exact = {(row.ticker, row.report_date): row for row in v2_rows}
    rows = []
    for ticker, observations in yahoo_by_ticker.items():
        for obs in observations:
            ...
            if v2 is None or v2.ebitda is None:
                continue
            rows.append({"provider_formula": "YAHOO_DIRECT_EBITDA", "ticker": ticker, "fiscal_year": v2.fiscal_year, "fiscal_period": v2.fiscal_period, "provider_period_date": obs.period_end_date, "canonical_date": v2.report_date, "match_mode": EXACT_DATE_INFERRED_FISCAL, "date_offset_days": 0, "v2_ebitda": v2.ebitda, "provider_candidate": obs.value, "absolute_difference": abs(obs.value - v2.ebitda), "relative_difference": _relative_difference(obs.value, v2.ebitda), "ratio": obs.value / v2.ebitda if v2.ebitda else "", "sec_context_type": ""})
    return rows


def _yahoo_inventory(conn: sqlite3.Connection) -> list[dict[str, Any]]:
    fields: dict[str, dict[str, Any]] = {}
    for row in conn.execute("SELECT symbol, quarterly_income_stmt_json FROM rc_fundamental_yahoo_raw WHERE market='usa' AND provider='yahoo' AND status='OK'"):
        try:
            payload = json.loads(str(row["quarterly_income_stmt_json"]))
        except Exception:
            continue
        columns = [str(value) for value in payload.get("columns", [])]
        index = [str(value) for value in payload.get("index", [])]
        ...
        for idx, field in enumerate(index):
            if field not in {PROVIDER_FIELD, "Normalized EBITDA", "Operating Income", "Reconciled Depreciation"}:
                continue
            ...
            for pos, _period in enumerate(columns):
                if pos < len(values) and values[pos] is not None:
                    ...
                    rec["periods"] += 1
    return [{"exact_field": field, "companies": len(rec["companies"]), "periods": rec["periods"], "units": "provider numeric reporting currency", "direct_provider_adjusted_semantics": "direct/provider-adjusted EBITDA" if field in {PROVIDER_FIELD, "Normalized EBITDA"} else "canonical component candidate", "candidate_null_gap_count": "", "previous_validation_metrics": "direct/normalized/derived EBITDA had divergent tails; use company-scoped tiers"} for field, rec in sorted(fields.items(), key=lambda item: item[1]["periods"], reverse=True)]


def _yahoo_formula_validation(conn: sqlite3.Connection) -> list[dict[str, Any]]:
    rows = []
    # Compact summary; detailed provider/V2 overlap is in ebitda_overlap_risk_validation.csv.
    rows.append({"formula": "Yahoo direct EBITDA vs canonical OI+D&A", "adjustment_classification": "company-dependent/provider-adjusted", "sign_anomalies": "present in provider history tails", "company_level_instability": "handled by company risk tiers", "notes": "Direct Yahoo EBITDA is deterministic but may include provider/company adjustments; only company-scoped tiers are implemented"})
    return rows


def _sec_inventory(conn: sqlite3.Connection) -> list[dict[str, Any]]:
    return [dict(row) for row in conn.execute("SELECT field_name AS concept, COUNT(DISTINCT ticker) AS companies, COUNT(*) AS periods, MIN(period_end_date) AS earliest, MAX(period_end_date) AS latest, statement_type AS candidate_type FROM rc_fundamental_statement_raw WHERE field_value IS NOT NULL AND ((statement_type='income' AND field_name LIKE '%OperatingIncome%') OR (field_name LIKE '%Depreciation%' OR field_name LIKE '%Amortization%')) GROUP BY field_name, statement_type ORDER BY periods DESC LIMIT 200")]


def _sec_context_rows(conn: sqlite3.Connection) -> list[dict[str, Any]]:
    return [{"provider": "SEC", "concept": row["concept"], "classification": "YTD_AMBIGUOUS", "reason": "legacy normalized cache does not expose enough raw duration/context fields for a narrow production EBITDA derivation in this phase", "rows": row["periods"]} for row in _sec_inventory(conn)]
