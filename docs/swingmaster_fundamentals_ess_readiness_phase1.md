...
Recommended next task: Phase 2, ticker identity contract hardening / audit.

This should be next because it is small and low risk. It validates the current `market + ticker` assumption before later valuation, rank, and ESS adapter work depends on it. It can be done without provider calls, schema changes, ESS integration, or real DB writes. It should focus on repo-evidence documentation and temp/read-only checks for ticker normalization, `osakedata` joins, `ticker_meta` joins, Yahoo symbol use, SEC CIK mapping boundaries, and visible ambiguity from tables that do not store `market`.
