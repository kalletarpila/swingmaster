...
from swingmaster.cli import run_fail10_sideways_eval
...
from swingmaster.research.fail10_sideways_evaluation import (
    EVAL_FILE_NAME,
    _compute_threshold_metrics,
    _compute_top_bucket_metrics,
    _recommend_candidate,
    evaluate_fail10_sideways_models,
)
...
def test_fail10_sideways_eval_uses_same_sideways_only_dataset_logic(tmp_path: Path) -> None:
    ...
    conn = sqlite3.connect(str(rc_db))
    eval_summary = evaluate_fail10_sideways_models(
        conn,
        model_dir=model_dir,
        regime_version=regime_version,
        computed_at="2026-03-07T00:00:00+00:00",
...
def test_fail10_sideways_eval_uses_same_feature_order_as_training(tmp_path: Path) -> None:
    rc_db = tmp_path / "rc.db"
    conn = sqlite3.connect(str(rc_db))
    apply_migrations(conn)
    regime_version = "REGIME_TEST_V1"
    ...
    feature_path = model_dir / "FAIL10_SIDEWAYS_FEATURE_LIST_V1.json"
...
def test_fail10_sideways_eval_computes_threshold_metrics() -> None:
...
def test_fail10_sideways_eval_computes_top_bucket_metrics() -> None:
...
def test_fail10_sideways_eval_cli_runs_and_writes_eval_json(monkeypatch, tmp_path: Path) -> None:
    rc_db = tmp_path / "rc.db"
    conn = sqlite3.connect(str(rc_db))
    apply_migrations(conn)
    regime_version = "REGIME_TEST_V1"
    ...
    run_fail10_sideways_eval.main()
    assert (out_dir / EVAL_FILE_NAME).exists()


def test_fail10_sideways_eval_recommendation_is_deterministic() -> None:
    kwargs = {
        "previous_selected": "FAIL10_SIDEWAYS_CATBOOST_V1",
    ...
    assert first == second
    assert first[0] == "FAIL10_SIDEWAYS_CATBOOST_V1"
