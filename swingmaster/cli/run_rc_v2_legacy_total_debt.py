...
from swingmaster.fundamentals_v2.legacy_total_debt import run_legacy_total_debt_import


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Offline legacy Yahoo Total Debt fallback importer for rc_fundamentals_v2")
    ...
    parser.add_argument("--apply", action="store_true", help="Apply NULL-only total_debt fills")
...
def main() -> None:
    ...
    if args.dry_run and args.apply:
        raise SystemExit("LEGACY_TOTAL_DEBT_DRY_RUN_AND_APPLY_MUTUALLY_EXCLUSIVE")
    if not args.dry_run and not args.apply:
        raise SystemExit("LEGACY_TOTAL_DEBT_REQUIRES_DRY_RUN_OR_APPLY")
    summary = run_legacy_total_debt_import(
    ...
    for key in ("mode", "provider_calls", "final_phase7_classification"):
