...
from swingmaster.cli import run_up20_sideways_training
...
def _insert_episode(
    ...
    *,
    ...
    conn.execute(
        f"""
        INSERT INTO rc_episode_exit_features ({", ".join(cols)})
        VALUES ({placeholders})
        """,
        values,
    )
...
def _seed_train_valid_test_sideways_dataset(conn: sqlite3.Connection, regime_version: str) -> None:
    ...
    rows = [
        ("EP2020A", "T2020A", "2020-01-10", 30.0, "SIDEWAYS"),
        ("EP2020B", "T2020B", "2020-02-10", 5.0, "SIDEWAYS"),
        ("EP2021A", "T2021A", "2021-01-10", 30.0, "SIDEWAYS"),
        ("EP2021B", "T2021B", "2021-02-10", 5.0, "SIDEWAYS"),
        ("EP2022A", "T2022A", "2022-01-10", 30.0, "SIDEWAYS"),
        ("EP2022B", "T2022B", "2022-02-10", 5.0, "SIDEWAYS"),
        ("EP2023A", "T2023A", "2023-01-10", 30.0, "SIDEWAYS"),
        ("EP2023B", "T2023B", "2023-02-10", 5.0, "SIDEWAYS"),
        ("EP2023C", "T2023C", "2023-03-10", 30.0, "SIDEWAYS"),
        ("EP2023D", "T2023D", "2023-04-10", 5.0, "SIDEWAYS"),
        ("EP2024A", "T2024A", "2024-01-10", 30.0, "SIDEWAYS"),
        ("EP2024B", "T2024B", "2024-02-10", 5.0, "SIDEWAYS"),
        ("EP2025A", "T2025A", "2025-01-10", 30.0, "SIDEWAYS"),
        ("EP2025B", "T2025B", "2025-02-10", 5.0, "SIDEWAYS"),
        ("EPBULL1", "TBULL1", "2023-05-10", 30.0, "BULL"),
        ("EPBULL2", "TBULL2", "2024-05-10", 5.0, "BULL"),
    ]
...
def test_up20_sideways_dataset_filters_to_sideways_regime_only() -> None:
...
def test_up20_sideways_dataset_uses_year_split_by_ew_exit_date() -> None:
    conn = sqlite3.connect(":memory:")
    apply_migrations(conn)
    regime_version = "REGIME_TEST_V1"
    _seed_train_valid_test_sideways_dataset(conn, regime_version)
...
def test_up20_sideways_training_excludes_non_feature_columns(tmp_path: Path, monkeypatch) -> None:
    conn = sqlite3.connect(":memory:")
    apply_migrations(conn)
    regime_version = "REGIME_TEST_V1"
    _seed_train_valid_test_sideways_dataset(conn, regime_version)
    ...
    def _fit_catboost_spy(x, y):  # type: ignore[no-untyped-def]
        ...
        assert "label_up20" not in x.columns
        return _FakeModel()
    ...
    monkeypatch.setattr(up20_sideways_training, "_fit_catboost", _fit_catboost_spy)
    monkeypatch.setattr(up20_sideways_training, "_fit_hgb", _fit_hgb_spy)
    monkeypatch.setattr(
        up20_sideways_training.joblib,
        "dump",
...
def test_up20_sideways_training_cli_runs_and_saves_both_artifacts(
    monkeypatch,
    tmp_path: Path,
) -> None:
    ...
    run_up20_sideways_training.main()
    ...
    assert (out_dir / "UP20_SIDEWAYS_FEATURE_LIST_V1.json").exists()
    comparison_path = out_dir / "UP20_SIDEWAYS_MODEL_COMPARISON_V1.json"
    assert comparison_path.exists()
    comparison = json.loads(comparison_path.read_text(encoding="utf-8"))
    assert comparison["selected_production_candidate"] in {MODEL_ID_CATBOOST, MODEL_ID_HGB}


def test_up20_sideways_training_comparison_selects_preferred_model_deterministically() -> None:
...
def test_up20_sideways_training_is_deterministic_given_same_inputs(tmp_path: Path) -> None:
    conn = sqlite3.connect(":memory:")
    apply_migrations(conn)
    regime_version = "REGIME_TEST_V1"
    _seed_train_valid_test_sideways_dataset(conn, regime_version)

    m1 = train_and_compare_up20_sideways(
        conn,
        out_dir=tmp_path / "run1",
        regime_version=regime_version,
        computed_at="2026-03-07T00:00:00+00:00",
    )
    m2 = train_and_compare_up20_sideways(
        conn,
