...
from swingmaster.cli import run_fail10_bull_training
...
def test_fail10_bull_dataset_filters_to_bull_regime_only() -> None:
...
def test_fail10_bull_dataset_uses_year_split_by_ew_exit_date() -> None:
    conn = sqlite3.connect(":memory:")
    apply_migrations(conn)
    regime_version = "REGIME_TEST_V1"
    _seed_train_valid_test_bull_dataset(conn, regime_version)
...
def test_fail10_bull_training_excludes_non_feature_columns(tmp_path: Path, monkeypatch) -> None:
    conn = sqlite3.connect(":memory:")
    apply_migrations(conn)
    regime_version = "REGIME_TEST_V1"
    _seed_train_valid_test_bull_dataset(conn, regime_version)
    ...
    monkeypatch.setattr(fail10_bull_training, "_fit_catboost", _fit_catboost_spy)
    monkeypatch.setattr(fail10_bull_training, "_fit_hgb", _fit_hgb_spy)
    monkeypatch.setattr(
        fail10_bull_training.joblib,
        "dump",
        lambda model, path: Path(path).write_text("fake-hgb", encoding="utf-8"),
    )

    metrics = train_and_compare_fail10_bull(
        conn,
...
def test_fail10_bull_training_cli_runs_and_saves_both_artifacts(monkeypatch, tmp_path: Path) -> None:
    ...
    assert (out_dir / "FAIL10_BULL_FEATURE_LIST_V1.json").exists()
    comparison_path = out_dir / "FAIL10_BULL_MODEL_COMPARISON_V1.json"
    assert comparison_path.exists()
    comparison = json.loads(comparison_path.read_text(encoding="utf-8"))
    assert comparison["selected_production_candidate"] in {MODEL_ID_CATBOOST, MODEL_ID_HGB}


def test_fail10_bull_training_comparison_selects_preferred_model_deterministically() -> None:
...
def test_fail10_bull_training_is_deterministic_given_same_inputs(tmp_path: Path) -> None:
    conn = sqlite3.connect(":memory:")
    apply_migrations(conn)
    regime_version = "REGIME_TEST_V1"
    _seed_train_valid_test_bull_dataset(conn, regime_version)
    m1 = train_and_compare_fail10_bull(
        conn,
        out_dir=tmp_path / "run1",
        regime_version=regime_version,
        computed_at="2026-03-07T00:00:00+00:00",
    )
    m2 = train_and_compare_fail10_bull(
        conn,
