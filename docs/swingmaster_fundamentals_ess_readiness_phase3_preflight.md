...
Phase 4A addresses `reported_fundamentals` point-in-time and vintage design, Phase 4B adds additive schema tables, Phase 4C1 adds a helper writer, Phase 4C2 adds a read-only helper, and Phase 4C3 adds a test-only adapter without changing current write paths or readers. The highest-value concepts are:
...
Those changes now have a Phase 4B schema foundation plus Phase 4C1/4C2/4C3 helper layers, but provider writes, production dual-write behavior, backfill, preflight population checks, and reader changes should still be implemented only in later, separately approved phases.
