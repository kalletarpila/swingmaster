...
from swingmaster.research.fail10_bull_evaluation import EVAL_FILE_NAME, FEATURE_LIST_FILE_NAME
from swingmaster.research.fail10_bull_training import MODEL_ID_CATBOOST, MODEL_ID_HGB
...
def _load_selected_candidate(model_dir: Path) -> str:
    ...
    if not path.exists():
        raise RuntimeError("FAIL10_BULL_EVAL_JSON_MISSING")
    payload = json.loads(path.read_text(encoding="utf-8"))
    value = payload.get("evaluation_recommended_candidate")
    if value not in {MODEL_ID_CATBOOST, MODEL_ID_HGB}:
        raise RuntimeError("FAIL10_BULL_SELECTED_CANDIDATE_INVALID")
    return str(value)
...
def _load_feature_list(model_dir: Path) -> list[str]:
    ...
    if not path.exists():
        raise RuntimeError("FAIL10_BULL_FEATURE_LIST_MISSING")
    payload = json.loads(path.read_text(encoding="utf-8"))
    cols = payload.get("feature_columns")
    if not isinstance(cols, list) or not cols or not all(isinstance(c, str) for c in cols):
        raise RuntimeError("FAIL10_BULL_FEATURE_LIST_INVALID")
    if tuple(cols) != tuple(FEATURE_COLUMNS):
        raise RuntimeError("FAIL10_BULL_FEATURE_ORDER_MISMATCH")
    return cols
...
def _load_model(model_dir: Path, model_id: str) -> tuple[Any, str, str]:
    if model_id == MODEL_ID_HGB:
        ...
        if not model_path.exists():
            raise RuntimeError("FAIL10_BULL_HGB_ARTIFACT_MISSING")
    ...
    if model_id == MODEL_ID_CATBOOST:
        ...
        if not model_path.exists():
            raise RuntimeError("FAIL10_BULL_CATBOOST_ARTIFACT_MISSING")
...
def compute_and_store_fail10_bull_scores(
    conn: sqlite3.Connection,
    *,
    model_dir: Path,
    ...
    for col in feature_columns:
        if not _column_exists(conn, "rc_episode_exit_features", col):
            raise RuntimeError(f"FAIL10_BULL_REQUIRED_FEATURE_COLUMN_MISSING_{col}")

    episodes_scanned = _count_scanned_episodes(conn, date_from=date_from, date_to=date_to)
    bull_count = _load_bull_episode_count(
        conn,
    ...
    if proba.shape[0] != score_frame.shape[0]:
        raise RuntimeError("FAIL10_BULL_SCORE_COUNT_MISMATCH")
