...
MODEL_ID_CATBOOST = "UP20_SIDEWAYS_CATBOOST_V1"
MODEL_ID_HGB = "UP20_SIDEWAYS_HGB_V1"
COMPARISON_ID = "UP20_SIDEWAYS_MODEL_COMPARISON_V1"


@dataclass(frozen=True)
class Up20SidewaysDataset:
    frame: pd.DataFrame
    feature_columns: tuple[str, ...]


@dataclass(frozen=True)
class Up20SidewaysMetrics:
    n_train: int
    n_valid: int
    n_test: int
...
def load_up20_sideways_dataset(conn: sqlite3.Connection, *, regime_version: str) -> Up20SidewaysDataset:
    ...
    if frame.empty:
        return Up20SidewaysDataset(frame=frame, feature_columns=tuple(FEATURE_COLUMNS))
...
def select_preferred_candidate(
    *,
    ...
    # Existing UP20 convention in repository favors CatBoost.
...
def train_and_compare_up20_sideways(
    ...
    *,
    ...
    y_train = train_df["label_up20"].astype(int)
    x_valid = valid_df[list(ds.feature_columns)]
    y_valid = valid_df["label_up20"].astype(int)
    x_test = test_df[list(ds.feature_columns)]
    y_test = test_df["label_up20"].astype(int)
    ...
    features_path = out_dir / "UP20_SIDEWAYS_FEATURE_LIST_V1.json"
