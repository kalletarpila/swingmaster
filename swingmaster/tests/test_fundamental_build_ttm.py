...
def test_build_ttm_null_handling_ignores_partial_nulls(tmp_path: Path) -> None:
    ...
    with sqlite3.connect(str(db_path)) as conn:
        ...
        row = conn.execute(
            """
            SELECT revenue_ttm, ebit_ttm, ebitda_ttm_missing, net_debt_to_ebit, net_debt_to_ebitda, share_dilution_yoy
            FROM (
                SELECT
                    revenue_ttm,
                    ebit_ttm,
...
def test_net_debt_to_ebit_is_null_when_ebit_missing(tmp_path: Path) -> None:
    ...
    with sqlite3.connect(str(db_path)) as conn:
        ...
        value = conn.execute(
            "SELECT net_debt_to_ebit FROM rc_fundamental_ttm WHERE ticker='AAPL' AND as_of_date='2024-12-31'"
        ).fetchone()[0]
        assert value is None


def test_net_debt_to_ebit_is_null_when_ebit_denominator_is_zero(tmp_path: Path) -> None:
...
def test_net_debt_to_ebit_handles_negative_net_debt(tmp_path: Path) -> None:
...
def test_build_ttm_target_scope_does_not_treat_adjacent_period_as_target(tmp_path: Path) -> None:
    db_path = tmp_path / "fundamental_ttm_target_scope_exact.db"
    run_migration(db_path)

    with sqlite3.connect(str(db_path)) as conn:
        for period in ["2025-03-31", "2025-06-30", "2025-09-30", "2026-01-05"]:
            _insert_quarterly_row(
                conn,
                "AAPL",
        ...
        conn.execute(
            ...
            SET revenue = 999.0
            WHERE ticker = 'AAPL'
              AND period_end_date = '2026-01-05'
            """
        )
        conn.commit()

        _quarterly_rows, ttm_rows_written, first_as_of_date, last_as_of_date = build_and_insert_ttm_rows(
            conn=conn,
            ticker="AAPL",
            run_id="TARGET_TTM",
            dry_run=False,
        ...
        assert first_as_of_date is None
        assert last_as_of_date is None
