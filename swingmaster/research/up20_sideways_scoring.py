...
from swingmaster.research.up20_sideways_evaluation import EVAL_FILE_NAME, FEATURE_LIST_FILE_NAME
from swingmaster.research.up20_sideways_training import MODEL_ID_CATBOOST, MODEL_ID_HGB
...
class Up20SidewaysScoreSummary:
...
def _load_selected_candidate(model_dir: Path) -> str:
    ...
    if not path.exists():
        raise RuntimeError("UP20_SIDEWAYS_EVAL_JSON_MISSING")
    payload = json.loads(path.read_text(encoding="utf-8"))
    value = payload.get("evaluation_recommended_candidate")
    if value not in {MODEL_ID_CATBOOST, MODEL_ID_HGB}:
        raise RuntimeError("UP20_SIDEWAYS_SELECTED_CANDIDATE_INVALID")
    return str(value)
...
def _load_feature_list(model_dir: Path) -> list[str]:
    ...
    if not path.exists():
        raise RuntimeError("UP20_SIDEWAYS_FEATURE_LIST_MISSING")
    payload = json.loads(path.read_text(encoding="utf-8"))
...
def compute_and_store_up20_sideways_scores(
    conn: sqlite3.Connection,
    *,
    model_dir: Path,
    ...
    for col in feature_columns:
        if not _column_exists(conn, "rc_episode_exit_features", col):
            raise RuntimeError(f"UP20_SIDEWAYS_REQUIRED_FEATURE_COLUMN_MISSING_{col}")
    ...
    if score_frame.empty:
        return Up20SidewaysScoreSummary(
    ...
    if proba.shape[0] != score_frame.shape[0]:
        raise RuntimeError("UP20_SIDEWAYS_SCORE_COUNT_MISMATCH")
