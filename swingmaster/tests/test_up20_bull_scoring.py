...
from swingmaster.cli import run_up20_bull_score
...
from swingmaster.research import up20_bull_training
from swingmaster.research.up20_bull_scoring import (
    SCORE_TABLE,
...
def _insert_episode(
    ...
    *,
    ...
    conn.execute(
        f"""
        INSERT INTO rc_episode_exit_features ({", ".join(feature_cols)})
        VALUES ({", ".join("?" for _ in feature_cols)})
        """,
        values,
    )
    conn.execute(
...
def _write_model_artifacts(model_dir: Path, feature_order: list[str] | None = None) -> None:
    model_dir.mkdir(parents=True, exist_ok=True)
    feature_columns = feature_order if feature_order is not None else list(FEATURE_COLUMNS)
    (model_dir / "UP20_BULL_FEATURE_LIST_V1.json").write_text(
        json.dumps({"feature_columns": feature_columns, "feature_count": len(feature_columns)}, indent=2, sort_keys=True)
        + "\n",
        encoding="utf-8",
    )
    joblib.dump(DummyProbModel(), model_dir / "UP20_BULL_HGB_V1.joblib")


def test_up20_bull_scoring_filters_to_bull_regime_only(tmp_path: Path) -> None:
    ...
    conn = sqlite3.connect(str(rc_db))
    summary = compute_and_store_up20_bull_hgb_scores(
        conn,
        model_dir=model_dir,
        regime_version=regime_version,
        mode="upsert",
    ...
    rows = conn.execute(
        f"SELECT episode_id FROM {SCORE_TABLE} ORDER BY episode_id ASC"
    ).fetchall()
    conn.close()

    assert summary.episodes_eligible == 2
    assert [str(r[0]) for r in rows] == ["EP_BULL_1", "EP_BULL_2"]


def test_up20_bull_scoring_uses_saved_feature_order(tmp_path: Path, monkeypatch) -> None:
    rc_db = tmp_path / "rc.db"
    conn = sqlite3.connect(str(rc_db))
    apply_migrations(conn)
    regime_version = "REGIME_TEST_V1"
    ...
    from swingmaster.research import up20_bull_scoring

    monkeypatch.setattr(up20_bull_scoring, "_load_hgb_model", lambda _: SpyModel())
...
def test_up20_bull_scoring_writes_probability_to_score_table(tmp_path: Path) -> None:
    rc_db = tmp_path / "rc.db"
    conn = sqlite3.connect(str(rc_db))
    apply_migrations(conn)
    regime_version = "REGIME_TEST_V1"
    ...
    rows = conn.execute(
        f"""
        SELECT episode_id, model_id, regime_used, predicted_probability
        FROM {SCORE_TABLE}
    ...
    assert len(rows) == 2
    assert all(str(r[1]) == "UP20_BULL_HGB_V1" for r in rows)
    assert all(str(r[2]) == "BULL" for r in rows)
    assert all(0.0 <= float(r[3]) <= 1.0 for r in rows)


def test_up20_bull_scoring_cli_is_idempotent(monkeypatch, tmp_path: Path) -> None:
    rc_db = tmp_path / "rc.db"
    conn = sqlite3.connect(str(rc_db))
    apply_migrations(conn)
    regime_version = "REGIME_TEST_V1"
    ...
    monkeypatch.setattr(run_up20_bull_score, "parse_args", lambda: args)

    run_up20_bull_score.main()
    run_up20_bull_score.main()
...
def test_up20_bull_scoring_fails_on_feature_order_mismatch(tmp_path: Path) -> None:
    rc_db = tmp_path / "rc.db"
    conn = sqlite3.connect(str(rc_db))
    apply_migrations(conn)
    regime_version = "REGIME_TEST_V1"
...
def test_up20_bull_scoring_does_not_retrain_model(tmp_path: Path, monkeypatch) -> None:
    rc_db = tmp_path / "rc.db"
    conn = sqlite3.connect(str(rc_db))
    apply_migrations(conn)
    regime_version = "REGIME_TEST_V1"
