...
from swingmaster.research.fail10_bear_evaluation import EVAL_FILE_NAME, FEATURE_LIST_FILE_NAME
from swingmaster.research.fail10_bear_training import MODEL_ID_CATBOOST, MODEL_ID_HGB
...
def _load_selected_candidate(model_dir: Path) -> str:
    ...
    if not path.exists():
        raise RuntimeError("FAIL10_BEAR_EVAL_JSON_MISSING")
    payload = json.loads(path.read_text(encoding="utf-8"))
    value = payload.get("evaluation_recommended_candidate")
    if value not in {MODEL_ID_CATBOOST, MODEL_ID_HGB}:
        raise RuntimeError("FAIL10_BEAR_SELECTED_CANDIDATE_INVALID")
    return str(value)
...
def _load_feature_list(model_dir: Path) -> list[str]:
    ...
    if not path.exists():
        raise RuntimeError("FAIL10_BEAR_FEATURE_LIST_MISSING")
    payload = json.loads(path.read_text(encoding="utf-8"))
    cols = payload.get("feature_columns")
    if not isinstance(cols, list) or not cols or not all(isinstance(c, str) for c in cols):
        raise RuntimeError("FAIL10_BEAR_FEATURE_LIST_INVALID")
    if tuple(cols) != tuple(FEATURE_COLUMNS):
        raise RuntimeError("FAIL10_BEAR_FEATURE_ORDER_MISMATCH")
    return cols
...
def _load_model(model_dir: Path, model_id: str) -> tuple[Any, str, str]:
    if model_id == MODEL_ID_HGB:
        ...
        if not model_path.exists():
            raise RuntimeError("FAIL10_BEAR_HGB_ARTIFACT_MISSING")
    ...
    if model_id == MODEL_ID_CATBOOST:
        ...
        if not model_path.exists():
            raise RuntimeError("FAIL10_BEAR_CATBOOST_ARTIFACT_MISSING")
...
def _load_bear_episode_count(
    ...
    *,
    ...
    row = conn.execute(
        ...
        WHERE p.entry_window_exit_date IS NOT NULL
          AND r.ew_exit_regime_combined = 'BEAR'
          {date_sql}
        """,
...
def compute_and_store_fail10_bear_scores(
    conn: sqlite3.Connection,
    *,
    model_dir: Path,
    ...
    for col in feature_columns:
        if not _column_exists(conn, "rc_episode_exit_features", col):
            raise RuntimeError(f"FAIL10_BEAR_REQUIRED_FEATURE_COLUMN_MISSING_{col}")

    episodes_scanned = _count_scanned_episodes(conn, date_from=date_from, date_to=date_to)
    bear_count = _load_bear_episode_count(
        conn,
    ...
    if int(score_frame.shape[0]) < bear_count:
        score_frame = _load_scoring_rows(
            conn,
            feature_columns=feature_columns,
            regime_version=regime_version,
    ...
    scores_skipped = max(0, bear_count - episodes_eligible)
    if score_frame.empty:
        return Fail10BearScoreSummary(
    ...
    if proba.shape[0] != score_frame.shape[0]:
        raise RuntimeError("FAIL10_BEAR_SCORE_COUNT_MISMATCH")
