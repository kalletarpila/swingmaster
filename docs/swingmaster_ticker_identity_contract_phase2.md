...
The contract is acceptable for the next low-risk phases, but it is not globally multi-market-safe. Several fundamentals tables remain ticker-only and should be interpreted as market-scoped by database/file context until a later migration design explicitly changes that.
...
3. For OMXH, the `.HE` suffix convention is currently used where relevant.
4. Any cross-DB join to `osakedata.db` must include `market` when reading prices or metadata.
5. Fundamentals core tables that are ticker-only must be treated as market-scoped by DB/file context, not globally multi-market-safe.
6. SEC CIK is a provider mapping used to fetch SEC data, not canonical SwingMaster security identity.
7. Yahoo `symbol` is a provider mapping/staging identifier, not canonical SwingMaster security identity.
8. Later ESS-facing reads should accept or produce `market + ticker`, not ticker-only identifiers.
9. No company/security-master should be added until a separate design proves `market + ticker` is insufficient.
...
Phase 2 test coverage now includes a same-ticker/two-market temp `osakedata` case to lock that price lookup is market-scoped.
...
Phase 2 test coverage now includes a temp metadata case showing that metadata from the wrong market is not attached.
...
| --- | --- | --- | --- | --- |
| Ticker collision across markets | Medium | Separate USA/FIN fundamentals DB paths and `market` filters in osakedata reads. | Does not block Phase 3 if documented; affects Phase 10 adapter design. | Add explicit `(market, ticker)` contract to adapter/preflight; consider schema changes only in a later migration design. |
| `.HE` suffix inference ambiguity | Medium | `infer_market()` and price behavior helpers use `.HE` convention; UI passes explicit market for core commands. | Does not block Phase 3. | Centralize/document normalization if later code starts sharing more identity logic. |
| Ticker-only primary keys in core tables | Medium | Market scoping is effectively by fundamentals DB/file context. | Does not block read-only preflight; must be addressed in PIT/vintage design. | Phase 4 migration design should decide whether to add `market` or preserve per-market DB separation. |
| Core tables without `market` | Medium | UI command builder selects USA vs FIN DB; valuation/percentile use market for external joins. | Does not block Phase 3; relevant for Phase 10. | Adapter output must include market from request/context, not infer it from table rows alone. |
...
- Phase 10 adapter contract: yes as the adapter key, but adapter output should always include explicit `market` and `ticker` and must not expose ticker-only table assumptions as globally safe.
