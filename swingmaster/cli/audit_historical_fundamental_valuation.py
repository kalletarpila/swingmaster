...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Audit current-style versus effective-date-safe historical valuation")
...
def main(argv: list[str] | None = None) -> int:
    ...
    with sqlite3.connect(str(Path(args.fundamentals_db).expanduser().resolve())) as fundamentals_conn, sqlite3.connect(
        ...
        summary, detail_rows = audit_historical_valuation(
            fundamentals_conn,
            price_conn,
            tickers=tickers,
    ...
    payload = {"summary": asdict_result(summary), "rows": detail_rows}
    write_json_atomic(output_json, payload)
    write_json_atomic(summary_json, payload["summary"])
    write_csv_atomic(output_csv, detail_rows)
    progress_log.parent.mkdir(parents=True, exist_ok=True)
    progress_log.write_text(f"rows={len(detail_rows)}\nstatus=ok\n", encoding="utf-8")
    if args.json:
        print(json.dumps({"summary": payload["summary"], "artifact_paths": {"output_json": str(output_json), "output_csv": str(output_csv), "output_summary_json": str(summary_json), "progress_log": str(progress_log)}}, indent=2, sort_keys=True))
    else:
