...
from swingmaster.fundamentals_v2.legacy_cash import run_legacy_cash_import


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Offline legacy Yahoo cash fallback importer for rc_fundamentals_v2")
    ...
    parser.add_argument("--apply", action="store_true", help="Apply NULL-only cash fills")
...
def main() -> None:
    ...
    if args.dry_run and args.apply:
        raise SystemExit("LEGACY_CASH_DRY_RUN_AND_APPLY_MUTUALLY_EXCLUSIVE")
    if not args.dry_run and not args.apply:
        raise SystemExit("LEGACY_CASH_REQUIRES_DRY_RUN_OR_APPLY")
    summary = run_legacy_cash_import(
    ...
    for key in ("mode", "provider_calls", "final_phase6_classification"):
