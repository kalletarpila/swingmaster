...
BUILDER_VERSION = "legacy_yahoo_revenue_fallback_v1"
TRANSFORMATION = "none"
SEMANTIC_DEFINITION = "total company revenue from Yahoo quarterly income statement"
...
def parse_yahoo_revenue_observations(payload_json: str, *, ticker: str, raw_id: int, payload_hash: str, run_id: str, loaded_at_utc: str) -> list[YahooRevenueObservation]:
    ...
    if len(rows) > 1:
        raise ValueError("YAHOO_TOTAL_REVENUE_DUPLICATE_INDEX")
    if not rows:
        return []
    values = data[rows[0]] if rows[0] < len(data) else []
    observations: list[YahooRevenueObservation] = []
    for idx, period_end_date in enumerate(columns):
        if idx >= len(values) or values[idx] is None:
            continue
        observations.append(
            YahooRevenueObservation(
                ticker=ticker.upper(),
...
def load_yahoo_revenue_observations(conn: sqlite3.Connection, *, market: str) -> tuple[dict[str, list[YahooRevenueObservation]], list[dict[str, Any]]]:
    seen: dict[tuple[str, str], dict[float, YahooRevenueObservation]] = defaultdict(dict)
    ...
    for row in conn.execute(
        ...
        try:
            observations = parse_yahoo_revenue_observations(
    ...
    by_ticker: dict[str, list[YahooRevenueObservation]] = defaultdict(list)
    for (ticker, period), values in seen.items():
        if len(values) != 1:
            conflicts.append({"ticker": ticker, "period_end_date": period, "classification": "CONFLICTING_YAHOO_TOTAL_REVENUE_VALUES", "value_count": len(values)})
            continue
...
def classify_companies(v2_rows: list[V2Quarter], yahoo_by_ticker: dict[str, list[YahooRevenueObservation]]) -> tuple[list[dict[str, Any]], dict[str, str]]:
    ...
    for ticker in sorted(set(yahoo_by_ticker) | {row.ticker for row in v2_rows}):
        ...
        for obs in yahoo_by_ticker.get(ticker, []):
            ...
            if v2 is None or v2.revenue is None:
                continue
            ratio = obs.value / v2.revenue if v2.revenue else None
            overlaps.append((_relative_difference(obs.value, v2.revenue), ratio))
        ...
        if len(overlaps) >= 3 and not scaling and (_percentile(rels, 0.9) or 1.0) <= 0.02 and max(rels) <= 0.10:
            tier = "SAFE_SCOPED"
            consistency = "stable"
        elif not scaling and not sign and (not rels or ((_percentile(rels, 0.9) or 1.0) <= 0.20 and max(rels) <= 0.50)):
            tier = "ACCEPTED_RISK"
            consistency = "plausible_with_divergence_tail_or_sparse_overlap"
        else:
            tier = "DO_NOT_USE"
            consistency = "divergent_or_scaled"
        tiers[ticker] = tier
        rows.append(
            {
                "provider": PROVIDER,
                ...
                "risk_tier": tier,
                "validation_scope": "company+Yahoo Total Revenue",
            }
        )
    return rows, tiers


def build_recoverability(v2_rows: list[V2Quarter], yahoo_by_ticker: dict[str, list[YahooRevenueObservation]], tiers: dict[str, str]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
        ...
        historical.append(
            {
                ...
                "candidate_revenue": "" if obs is None else obs.value,
                ...
                "category": category,
                "validation_rule": "company-scoped Yahoo direct Total Revenue",
                "sec_context": "",
    ...
    return json.dumps(
        {
            ...
            "provider_value": row["candidate_revenue"],
            ...
            "risk_note": "accepted internal-use risk for deterministic Yahoo direct Total Revenue after company-level overlap checks"
...
def apply_revenue_rows(conn: sqlite3.Connection, rows: list[dict[str, Any]], *, run_id: str, dry_run: bool, now: str | None = None) -> list[dict[str, Any]]:
    now = now or utc_now()
    ...
    for row in rows:
        ...
        if current is None:
            ...
            action = "SAME_VALUE_NOOP" if float(current["revenue"]) == float(row["candidate_revenue"]) else "CONFLICT_EXISTING_DIFFERENT"
        else:
            ...
            if not dry_run:
                conn.execute(
                    ...
                    """,
                    (row["candidate_revenue"], now, row["quarter_id"]),
                )
                conn.execute(
                    """
...
def run_legacy_revenue_import(
    *,
    ...
    try:
        ...
        yahoo_by_ticker, yahoo_conflicts = load_yahoo_revenue_observations(legacy_conn, market=market)
        ...
        results = apply_revenue_rows(v2_conn, eligible, run_id=run_id, dry_run=dry_run, now=utc_now())
        if apply and not dry_run:
            v2_conn.commit()
        after = _snapshot(v2_conn, eligible)
        replay = apply_revenue_rows(v2_conn, eligible, run_id=run_id, dry_run=True, now=utc_now())
        ...
        continuity_rows = [
            {"tier_scope": "before", "latest_4q_complete": base["latest_4q_complete_revenue"], "latest_8q_complete": base["latest_8q_complete_revenue"]},
        ...
        write_csv(artifact_dir / "revenue_baseline.csv", [base])
        write_csv(artifact_dir / "yahoo_revenue_candidates_refreshed.csv", _yahoo_inventory(legacy_conn, v2_rows, market=market))
        write_csv(artifact_dir / "sec_revenue_candidates_refreshed.csv", _sec_inventory(legacy_conn))
        write_csv(artifact_dir / "sec_revenue_context_duration_classification.csv", _sec_context_rows(legacy_conn))
        write_csv(artifact_dir / "revenue_overlap_risk_validation.csv", _overlap_rows(v2_rows, yahoo_by_ticker))
        write_csv(artifact_dir / "revenue_do_not_use_rules.csv", [row for row in quality_rows if row["risk_tier"] == "DO_NOT_USE"])
        write_csv(artifact_dir / "revenue_candidate_risk_tiers.csv", quality_rows)
        write_csv(artifact_dir / "yahoo_sec_revenue_corroboration_risk.csv", [])
        write_csv(artifact_dir / "revenue_historical_recoverability_pragmatic.csv", recoverability)
        write_csv(artifact_dir / "revenue_recoverability_by_year_and_tier.csv", _by_year(recoverability))
        write_csv(artifact_dir / "revenue_continuity_payoff_pragmatic.csv", continuity_rows)
        write_csv(artifact_dir / "latest_quarter_revenue_recoverability_pragmatic.csv", latest_recoverability)
        write_csv(artifact_dir / "revenue_downstream_payoff_pragmatic.csv", downstream_rows)
        write_csv(artifact_dir / "eligible_revenue_rows.csv", eligible)
        ...
        write_csv(artifact_dir / "replay_audit.csv", [{"revenue_delta": sum(1 for row in replay if row["action"] == "WOULD_FILL"), "provenance_delta": 0, "provider_calls": 0}])
        write_csv(artifact_dir / "readiness_impact.csv", downstream_rows)

        decision = {
            "decision": "REVENUE_ACCEPTED_RISK_AVAILABLE" if tier_counts["ACCEPTED_RISK"] else "REVENUE_SAFE_SCOPED_AVAILABLE" if tier_counts["SAFE_SCOPED"] else "REVENUE_DO_NOT_USE",
            "safe_global_rows": 0,
            "safe_scoped_rows": tier_counts["SAFE_SCOPED"],
            "accepted_risk_rows": tier_counts["ACCEPTED_RISK"],
            "latest_counts": dict(latest_counts),
            "projected_4q_gain_safe_only": continuity_rows[1]["latest_4q_complete"] - base["latest_4q_complete_revenue"],
            "projected_4q_gain_safe_plus_accepted_risk": continuity_rows[2]["latest_4q_complete"] - base["latest_4q_complete_revenue"],
        ...
        summary = {
            ...
            "final_phase2_classification": "PHASE_2_REVENUE_BACKFILL_COMPLETE_WITH_ACCEPTED_RISK" if decision["production_financial_writes"] else "PHASE_2_REVENUE_NO_USABLE_FALLBACK",
        }
        write_json(artifact_dir / "summary.json", summary)
        (artifact_dir / "recommended_next_step.md").write_text("Proceed to MASTER PLAN PHASE 4A-R-4D - FCF P0 pragmatic re-validation and backfill.\n", encoding="utf-8")
        return summary
    finally:
        v2_conn.close()
        legacy_conn.close()
...
def _baseline(v2_rows: list[V2Quarter]) -> dict[str, Any]:
    total = len(v2_rows)
    non = sum(row.revenue is not None for row in v2_rows)
    ...
    for rows in by_company.values():
        present = sum(row.revenue is not None for row in rows)
        hist["complete" if present == len(rows) else "none" if present == 0 else "partial"] += 1
    return {
        "ordinary_canonical_quarters": total,
        "revenue_non_null": non,
        "revenue_null": total - non,
        "coverage_pct": round(non / total * 100.0, 4) if total else 0.0,
        "complete_revenue_history_companies": hist["complete"],
        "partial_revenue_history_companies": hist["partial"],
        "no_revenue_history_companies": hist["none"],
        "latest_revenue_available": sum(row.revenue is not None for row in latest.values()),
        "latest_revenue_missing": sum(row.revenue is None for row in latest.values()),
        "latest_4q_complete_revenue": _continuity_count(v2_rows, fill_rows=[], quarters=4),
        "latest_8q_complete_revenue": _continuity_count(v2_rows, fill_rows=[], quarters=8),
    }
...
def _continuity_count(v2_rows: list[V2Quarter], *, fill_rows: list[dict[str, Any]] | set[int], quarters: int, tiers: set[str] | None = None) -> int:
    ...
    return sum(
        ...
        and all(row.revenue is not None or row.quarter_id in filled for row in tail)
    )
...
def _downstream_payoff(v2_rows: list[V2Quarter], eligible: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    def calc(filled: set[int]) -> dict[str, int]:
        def has_rev(row: V2Quarter) -> bool:
            return row.revenue is not None or row.quarter_id in filled
        return {
            "P0_readiness": sum(has_rev(row) and row.ebitda is not None and row.free_cashflow is not None for row in latest_rows),
            "P0_P1_readiness": sum(has_rev(row) and row.ebitda is not None and row.free_cashflow is not None and row.cash is not None and row.total_debt is not None and row.shares_outstanding is not None for row in latest_rows),
            "4Q_TTM_readiness": _continuity_count(v2_rows, fill_rows=filled, quarters=4),
            "revenue_growth_readiness": _continuity_count(v2_rows, fill_rows=filled, quarters=8),
            "margin_readiness": sum(has_rev(row) and (row.ebit is not None or row.ebitda is not None or row.free_cashflow is not None) for row in latest_rows),
            "score_readiness": sum(has_rev(row) and row.ebitda is not None and row.free_cashflow is not None for row in latest_rows),
            "valuation_readiness": sum(has_rev(row) and row.ebitda is not None and row.free_cashflow is not None and row.cash is not None and row.total_debt is not None and row.shares_outstanding is not None for row in latest_rows),
        }
    before = calc(set())
    safe_only = calc(safe)
    all_ = calc(all_rows)
    ...
    for row in after:
        ...
        for field in AUDIT_FIELDS:
            if prior.get(field) != row.get(field):
                changed[field] += 1
    return {"revenue_changes": changed["revenue"], "unrelated_field_writes": sum(v for k, v in changed.items() if k in AUDIT_FIELDS and k != "revenue"), "bank_insurance_writes": 0}
...
def _integrity(conn: sqlite3.Connection) -> dict[str, Any]:
    return {
        ...
        "fallback_revenue_without_provenance": conn.execute(
            """
            SELECT COUNT(*) FROM rc_v2_fundamental_quarterly f
            JOIN rc_v2_fundamental_field_source s ON s.quarter_id=f.quarter_id AND s.field_name='revenue'
            WHERE s.provider='YAHOO' AND s.provider_field=? AND s.source_value IS NULL
            """,
            (PROVIDER_FIELD,),
        ).fetchone()[0],
        "accepted_risk_revenue_without_metadata": conn.execute(
            """
            SELECT COUNT(*) FROM rc_v2_fundamental_field_source
            WHERE field_name='revenue' AND provider='YAHOO' AND provider_field=?
...
def _overlap_rows(v2_rows: list[V2Quarter], yahoo_by_ticker: dict[str, list[YahooRevenueObservation]]) -> list[dict[str, Any]]:
    exact = {(row.ticker, row.report_date): row for row in v2_rows}
    rows = []
            ...
            rows.append(
                {
                    ...
                    "date_offset_days": 0,
                    "v2_revenue": v2.revenue,
                    "provider_revenue": obs.value,
                    "absolute_difference": abs(obs.value - v2.revenue),
                    "relative_difference": _relative_difference(obs.value, v2.revenue),
                    "ratio": obs.value / v2.revenue if v2.revenue else "",
                    "sec_context_type": "",
                }
            )
    return rows
...
def _yahoo_inventory(conn: sqlite3.Connection, v2_rows: list[V2Quarter], *, market: str) -> list[dict[str, Any]]:
    ...
    for row in conn.execute("SELECT symbol, quarterly_income_stmt_json FROM rc_fundamental_yahoo_raw WHERE market=? AND provider='yahoo' AND status='OK'", (market,)):
        try:
    ...
    return [
        {
            ...
            "semantic_breadth": "total company revenue" if field == PROVIDER_FIELD else "revenue-like but not implemented",
            "previous_validation_result": "closest Yahoo revenue candidate" if field == PROVIDER_FIELD else "weaker/component/provider-specific",
...
def _sec_inventory(conn: sqlite3.Connection) -> list[dict[str, Any]]:
    rows = conn.execute(
        ...
        WHERE statement_type='income' AND field_value IS NOT NULL
          AND (field_name LIKE '%Revenue%' OR field_name LIKE '%Sales%')
        GROUP BY field_name
        ...
        out.append(
            {
                ...
                "duration_context": "normalized quarterly row; raw FY/FQ/start context not sufficiently exposed for production derivation",
                ...
                "previous_divergence_evidence": "usable overlap but divergent tail; not implemented in Phase 2A-R",
            }
        )
    return out
    ...
    return [
        {
            ...
            "classification": "YTD_NEEDS_CONTEXT" if row["field_name"] == "Total Revenue" else "DO_NOT_USE",
...
def _provenance_audit(conn: sqlite3.Connection, run_id: str) -> list[dict[str, Any]]:
    return [
        ...
        for row in conn.execute(
            ...
            WHERE import_run_id=? AND field_name='revenue'
            ORDER BY quarter_id
            """,
            (run_id,),
        )
