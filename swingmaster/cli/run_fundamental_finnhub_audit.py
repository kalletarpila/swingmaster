...
from swingmaster.fundamentals.providers.finnhub import FinnhubClient
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run deterministic Finnhub Finland fundamentals raw audit")
    ...
    parser.add_argument("--symbols", default=None, help="Comma-separated Finnhub symbols")
...
def payload_has_usable_report_data(payload: dict[str, Any]) -> bool:
    ...
    for item in data:
        if not isinstance(item, dict):
            continue
        for key in ("report", "statement", "statements"):
            if _has_non_empty_value(item.get(key)):
                return True
    return False
...
def insert_audit_rows(conn: sqlite3.Connection, rows: list[dict[str, Any]]) -> int:
    conn.executemany(
        """
        INSERT INTO rc_fundamental_finnhub_raw (
            market,
            provider,
            symbol,
            description,
...
def run_finnhub_audit(
    ...
    for symbol in symbols:
        try:
            payload = client.get_financials_reported(symbol=symbol, freq=DEFAULT_FREQ)
            status = "OK" if payload_has_usable_report_data(payload) else "EMPTY"
            if status == "OK":
                ok_count += 1
            else:
                empty_count += 1
