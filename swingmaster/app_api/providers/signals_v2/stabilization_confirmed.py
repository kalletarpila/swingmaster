...
def eval_stabilization_confirmed(
    ...
    if recent_median > baseline_median * RANGE_SHRINK_RATIO:
        return False
    ...
    for r in recent_range:
        if r >= baseline_median * WIDE_DAY_MULT:
            wide_days += 1
    if (wide_days / float(STAB_WINDOW)) > WIDE_DAY_RATIO_MAX:
        return False
    ...
    if significant_new_low_count != 0:
        return False
    if sweep_count > SWEEP_MAX_COUNT:
        return False
    ...
    for d in range(STAB_WINDOW):
        ...
        if pos >= CLOSE_UPPER_FRAC_MIN:
            upper_closes += 1
    if upper_closes < CLOSE_UPPER_DAYS_MIN:
        return False
