...
from swingmaster.research.up20_bear_training import (
    COMPARISON_ID,
    MODEL_ID_CATBOOST,
    MODEL_ID_HGB,
    Up20BearDataset,
    load_up20_bear_dataset,
)

EVAL_ID = "UP20_BEAR_MODEL_EVAL_V1"
EVAL_FILE_NAME = f"{EVAL_ID}.json"
FEATURE_LIST_FILE_NAME = "UP20_BEAR_FEATURE_LIST_V1.json"
...
def _load_feature_list(model_dir: Path) -> list[str]:
    ...
    if not isinstance(cols, list) or not all(isinstance(x, str) for x in cols):
        raise RuntimeError("UP20_BEAR_FEATURE_LIST_INVALID")
    return cols
...
def _load_models(model_dir: Path) -> tuple[CatBoostClassifier, Any]:
    ...
    if not cat_path.exists():
        raise RuntimeError("UP20_BEAR_CATBOOST_ARTIFACT_MISSING")
    if not hgb_path.exists():
        raise RuntimeError("UP20_BEAR_HGB_ARTIFACT_MISSING")
...
def _build_eval_frame(
    ...
    *,
    ...
    ds = load_up20_bear_dataset(conn, regime_version=regime_version)
    if ds.frame.empty:
        raise RuntimeError("UP20_BEAR_DATASET_EMPTY")
    if tuple(feature_order_from_artifact) != ds.feature_columns:
        raise RuntimeError("UP20_BEAR_FEATURE_ORDER_MISMATCH")
    return ds


def evaluate_up20_bear_models(
    conn: sqlite3.Connection,
    *,
    model_dir: Path,
    ...
    if valid_df.empty or test_df.empty:
        raise RuntimeError("UP20_BEAR_SPLIT_EMPTY")
