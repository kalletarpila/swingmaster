...
from swingmaster.cli import run_fail10_bear_training
...
from swingmaster.research import fail10_bear_training
from swingmaster.research.fail10_bear_training import (
    MODEL_ID_CATBOOST,
    MODEL_ID_HGB,
    load_fail10_bear_dataset,
    select_preferred_candidate,
    train_and_compare_fail10_bear,
)
...
def _seed_train_valid_test_bear_dataset(conn: sqlite3.Connection, regime_version: str) -> None:
    _create_pipeline_episode_table(conn)
    rows = [
        ("EP2020A", "T2020A", "2020-01-10", 5.0, "BEAR"),
        ("EP2020B", "T2020B", "2020-02-10", 30.0, "BEAR"),
        ("EP2021A", "T2021A", "2021-01-10", 5.0, "BEAR"),
        ("EP2021B", "T2021B", "2021-02-10", 30.0, "BEAR"),
        ("EP2022A", "T2022A", "2022-01-10", 5.0, "BEAR"),
        ("EP2022B", "T2022B", "2022-02-10", 30.0, "BEAR"),
        ("EP2023A", "T2023A", "2023-01-10", 5.0, "BEAR"),
        ("EP2023B", "T2023B", "2023-02-10", 30.0, "BEAR"),
        ("EP2023C", "T2023C", "2023-03-10", 5.0, "BEAR"),
        ("EP2023D", "T2023D", "2023-04-10", 30.0, "BEAR"),
        ("EP2024A", "T2024A", "2024-01-10", 5.0, "BEAR"),
        ("EP2024B", "T2024B", "2024-02-10", 30.0, "BEAR"),
        ("EP2025A", "T2025A", "2025-01-10", 5.0, "BEAR"),
        ("EP2025B", "T2025B", "2025-02-10", 30.0, "BEAR"),
...
def test_fail10_bear_dataset_filters_to_bear_regime_only() -> None:
    conn = sqlite3.connect(":memory:")
    apply_migrations(conn)
    regime_version = "REGIME_TEST_V1"
    _seed_train_valid_test_bear_dataset(conn, regime_version)
    ds = load_fail10_bear_dataset(conn, regime_version=regime_version)
...
def test_fail10_bear_dataset_uses_year_split_by_ew_exit_date() -> None:
    conn = sqlite3.connect(":memory:")
    apply_migrations(conn)
    regime_version = "REGIME_TEST_V1"
    _seed_train_valid_test_bear_dataset(conn, regime_version)
...
def test_fail10_bear_training_excludes_non_feature_columns(tmp_path: Path, monkeypatch) -> None:
    conn = sqlite3.connect(":memory:")
    apply_migrations(conn)
    regime_version = "REGIME_TEST_V1"
    _seed_train_valid_test_bear_dataset(conn, regime_version)
    ...
    monkeypatch.setattr(fail10_bear_training, "_fit_catboost", _fit_catboost_spy)
    monkeypatch.setattr(fail10_bear_training, "_fit_hgb", _fit_hgb_spy)
    monkeypatch.setattr(
        fail10_bear_training.joblib,
        "dump",
        lambda model, path: Path(path).write_text("fake-hgb", encoding="utf-8"),
    )

    metrics = train_and_compare_fail10_bear(
        conn,
...
def test_fail10_bear_training_cli_runs_and_saves_both_artifacts(monkeypatch, tmp_path: Path) -> None:
    ...
    assert (out_dir / "FAIL10_BEAR_FEATURE_LIST_V1.json").exists()
    comparison_path = out_dir / "FAIL10_BEAR_MODEL_COMPARISON_V1.json"
    assert comparison_path.exists()
    comparison = json.loads(comparison_path.read_text(encoding="utf-8"))
    assert comparison["selected_production_candidate"] in {MODEL_ID_CATBOOST, MODEL_ID_HGB}


def test_fail10_bear_training_comparison_selects_preferred_model_deterministically() -> None:
...
def test_fail10_bear_training_is_deterministic_given_same_inputs(tmp_path: Path) -> None:
    conn = sqlite3.connect(":memory:")
    apply_migrations(conn)
    regime_version = "REGIME_TEST_V1"
    _seed_train_valid_test_bear_dataset(conn, regime_version)
    m1 = train_and_compare_fail10_bear(
        conn,
        out_dir=tmp_path / "run1",
        regime_version=regime_version,
        computed_at="2026-03-07T00:00:00+00:00",
    )
    m2 = train_and_compare_fail10_bear(
        conn,
