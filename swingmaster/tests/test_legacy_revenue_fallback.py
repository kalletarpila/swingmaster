...
from swingmaster.fundamentals_v2.legacy_revenue import (
    PROVIDER_FIELD,
    apply_revenue_rows,
    parse_yahoo_revenue_observations,
)


def test_parse_yahoo_revenue_observations_keeps_explicit_zero_and_skips_missing() -> None:
    payload = {
    ...
    rows = parse_yahoo_revenue_observations(
        json.dumps(payload),
        ticker="abc",
        raw_id=1,
        payload_hash="hash",
...
def test_apply_revenue_rows_fills_null_with_accepted_risk_provenance() -> None:
    conn = _memory_v2()

    result = apply_revenue_rows(conn, [_eligible_row(value=123.0, tier="ACCEPTED_RISK")], run_id="rev_run", dry_run=False, now="2026-01-01T00:00:00Z")
    ...
    source = conn.execute("SELECT source_value FROM rc_v2_fundamental_field_source WHERE quarter_id=1 AND field_name='revenue'").fetchone()[0]
...
def test_apply_revenue_rows_does_not_overwrite_existing_revenue() -> None:
    conn = _memory_v2(existing_revenue=99.0)

    result = apply_revenue_rows(conn, [_eligible_row(value=123.0)], run_id="rev_run", dry_run=False)
...
def test_apply_revenue_rows_dry_run_has_no_writes() -> None:
    conn = _memory_v2()

    result = apply_revenue_rows(conn, [_eligible_row(value=123.0)], run_id="rev_run", dry_run=True)

    assert result[0]["action"] == "WOULD_FILL"
    assert conn.execute("SELECT revenue FROM rc_v2_fundamental_quarterly WHERE quarter_id=1").fetchone()[0] is None
    assert conn.execute("SELECT COUNT(*) FROM rc_v2_fundamental_field_source").fetchone()[0] == 0
