...
from swingmaster.research.up20_bear_evaluation import EVAL_FILE_NAME, FEATURE_LIST_FILE_NAME
from swingmaster.research.up20_bear_training import MODEL_ID_CATBOOST, MODEL_ID_HGB
...
def _load_feature_list(model_dir: Path) -> list[str]:
    ...
    if not path.exists():
        raise RuntimeError("UP20_BEAR_FEATURE_LIST_MISSING")
    payload = json.loads(path.read_text(encoding="utf-8"))
...
def compute_and_store_up20_bear_scores(
    conn: sqlite3.Connection,
    *,
    model_dir: Path,
    ...
    for col in feature_columns:
        if not _column_exists(conn, "rc_episode_exit_features", col):
            raise RuntimeError(f"UP20_BEAR_REQUIRED_FEATURE_COLUMN_MISSING_{col}")
    ...
    if proba.shape[0] != score_frame.shape[0]:
        raise RuntimeError("UP20_BEAR_SCORE_COUNT_MISMATCH")
